...
/// Independent view for Working Memory Task.
struct WorkingMemoryTaskView: View {
    ...
    var body: some View {
        Group {
            // Verify this is actually a working memory task
            if viewModel.isWorkingMemoryTask {
                ...
                VStack(spacing: 16) {
                    Text("Error: Wrong task type")
                        .font(.headline)
                        .foregroundColor(.red)
                    Text("This view is for Working Memory Task, but a different task is active.")
                        .font(.subheadline)
        ...
        .onAppear {
            ...
            if !viewModel.isWorkingMemoryTask {
                print("⚠️ WorkingMemoryTaskView: Warning - current task is not a WorkingMemoryTask!")
            ...
            if viewModel.currentState == .completed && viewModel.isWorkingMemoryTask {
            ...
            HStack {
                ...
                if let score = response.score {
                    Text("\(Int(score))/5")
                        ...
                        .foregroundColor(scoreColor(score / 5.0))
                }
            }
